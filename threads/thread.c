...
//wake time comperator
bool thread_wake_comp (const struct list_elem* a, const struct list_elem*b, void* aux UNUSED){
    return list_entry (a, struct thread, wait_elem)->wake_tick < list_entry (b, struct thread, wait_elem)->wake_tick;
}
...
thread_set_priority (int new_priority) 
{
    ...
    if(list_empty(&t->lock_list) || new_priority > t->priority)
